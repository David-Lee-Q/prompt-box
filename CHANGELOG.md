...
- 场景抽屉点选后自动收起；移动端场景操作按钮常驻可见，规避触屏粘滞 hover 误触

### 文档与版本

- README 更新至 v2.3（功能特性、项目结构、部署说明）
- 版本号对齐（package.json / 扩展 manifest / 前端状态栏版本）
...
- 账号体系：注册/登录，本地账号仅当前设备有效，云端账号跨设备登录
- 新设备用同一云端账号登录且本地为空时自动恢复云端数据
- 数据管理页（`/data`）：本地统计、全量 JSON 备份/导入、同步到云端/从云端恢复、云端更新时间
- 冲突处理：Last-Write-Wins，云端存在更新版本时覆盖需人工确认
- 零依赖同步后端（Node 原生 HTTP，JSON 文件存储，`SYNC_PORT` 可配），`pnpm dev` 并发拉起前端 + 后端
- 无同步后端时自动降级为纯本地模式

### 修复

- 数据管理页切换页面焦点后自动刷新云端状态
- 统一登录认证：本地账号登录失败时回退云端账号登录
...
- Tab 键在提示词编辑器中改为光标处插入缩进，而非始终在行首缩进（自定义 `smartTab` 键绑定）
